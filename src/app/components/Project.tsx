...
export default function Project({ name, description, skills, imageSrc }:
    ...
    return (
        <Reveal>
            <div className="w-80 h-96 bg-background-3 bg-opacity-25 backdrop-blur-sm shadow-md shadow-background-1 hover:shadow-glow-lg hover:shadow-secondary
                    ...
                    <div className="flex flex-col">
                        ...
                        <div className="flex flex-wrap gap-1 mb-2">
                            {skills.map((skill, index) => (
                                <span key={index} className={`text-secondary bg-background-4 bg-opacity-25 px-2 rounded-lg h6`}>{skill}</span>
                            ))}
                        </div>
